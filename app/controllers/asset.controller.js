...
const checkSerialNumber = (req, res) => {

  var where_query = { 'is_active': true }

  let serial_number = req.body.serial_number || req.query.serial_number || req.params.serial_number;

  if (!serial_number || serial_number.trim() === '') {
    return res.status(200).send({
      message: "Serial number must be filled",
      data: null
    });
  }
  ...
  if (serial_number) {
    where_query = {
      ...where_query,
      serial_number: where(fn('LOWER', col('assets.serial_number')), fn('LOWER', serial_number))
    }
  }
