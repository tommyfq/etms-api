...
async function update (req,res) {
    const existItem = await Part.findOne({
        where:{
          part_name: {
...
async function create (req,res){
  const existItem = await Part.findOne({
      where:{
          part_name: where(fn('LOWER', col('part_name')), fn('LOWER', req.body.part_name)),
      }
  });
  ...
  try{
      ...
      await Part.create(data,{transaction: t});
...
const upload = async(req, res) => {
  ...
  try{
    ...
    const sheetExists = sheets.includes('part');
    ...
    var result = [];
      ...
      let temp = xlsx.utils.sheet_to_json(file.Sheets['part'])
      if(temp.length > 100){
...
const download = async(req, res) => {
  ...
  try {
    ...
    xlsx.utils.book_append_sheet(workbook, worksheet, 'part');
    ...
    res.setHeader('Content-Disposition', 'attachment; filename="part_data.xlsx"');
