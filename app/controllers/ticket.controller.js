...
const download = async (req, res) => {
  ...
  if (req.body.hasOwnProperty("filter_year")) {
    if (typeof req.body.filter_year === "string") {
      if (req.body.filter_year != "") {
        where_query += ` AND EXTRACT(YEAR FROM ticket."createdAt") = ${req.body.filter_year}`
      }
    }
  }
  ...
  if (req.body.hasOwnProperty("filter_month")) {
    if (typeof req.body.filter_month === "string") {
      if (req.body.filter_month != "") {
        where_query += ` AND EXTRACT(MONTH FROM ticket."createdAt") = ${req.body.filter_month}`
      }
    }
  }
  ...
  try {
    ...
    if (result.length === 0) {
      return res.status(200).send({
        message: "No Data Found in Ticket"
      });
    } else {
      const formattedResult = result.map((item, index) => {
        const createdAt = item.createdAt ? moment(item.createdAt).utcOffset(7).format('YYYY-MM-DD') : ""
        ...
        return {
          ...
          'Case Category': item.case_category,
          'Part': item.part_name
        }
      });

      const workbook = reader.utils.book_new();
      const worksheet = reader.utils.json_to_sheet(formattedResult);
      reader.utils.book_append_sheet(workbook, worksheet, 'tickets');
      const excelBuffer = reader.write(workbook, { bookType: 'xlsx', type: 'buffer' });
      const timestamp = moment().format("YYYYMMDDHHmmss");

      res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
      res.setHeader('Content-Disposition', 'attachment; filename="tickets_' + timestamp + '.xlsx"');
